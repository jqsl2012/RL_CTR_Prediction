...
import src.models.Hybrid_SAC_model as sac_model
...
def train(rl_model, model_dict, data_loader, embedding_layer, exploration_rate, device):
    ...
    for i, (features, labels) in enumerate(tqdm.tqdm(data_loader, smoothing=0, mininterval=1.0)):
        ...
        c_actions, ensemble_c_actions, ensemble_d_actions = rl_model.choose_action(embedding_vectors)
        ...
        transitions = torch.cat([features.float(), c_actions, ensemble_d_actions.float(), rewards], dim=1)
...
def test(rl_model, model_dict, embedding_layer, data_loader, loss, device):
    ...
    with torch.no_grad():
        for i, (features, labels) in enumerate(data_loader):
            ...
            prob_weights, actions = rl_model.choose_best_action(embedding_vectors)
